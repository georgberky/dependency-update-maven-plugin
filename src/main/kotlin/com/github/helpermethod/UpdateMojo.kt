...
class UpdateMojo: AbstractMojo() {
    ...
    private fun dependencyManagementUpdates() =
        (mavenProject.originalModel.dependencyManagement?.dependencies ?: listOf())
            ...
            .mapNotNull { artifact ->
                update(artifact) { version, pom ->
                    pom.selectFirst("project > dependencyManagement > dependencies > dependency:has(> groupId:containsOwn(${artifact.groupId})):has(> artifactId:containsOwn(${artifact.artifactId})):has(> version:containsOwn(${artifact.version})) > version").text(version)
                }
            }
    ...
    private fun dependencyUpdates() =
        mavenProject.originalModel.dependencies
            ...
            .mapNotNull { artifact ->
                update(artifact) { version, pom ->
                    pom.selectFirst("project > dependencies > dependency:has(> groupId:containsOwn(${artifact.groupId})):has(> artifactId:containsOwn(${artifact.artifactId})):has(> version:containsOwn(${artifact.version})) > version").text(version)
                }
            }
